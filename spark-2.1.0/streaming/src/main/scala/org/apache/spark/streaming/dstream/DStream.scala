...
/**
 * DStream是spark streaming的最基本抽象, 是一个RDD的连续的序列,表示连续的数据流
 * (seeorg.apache.spark.rdd.RDD in the Spark core documentation for more details on RDDs).
 ...
 * parent DStream.
 *
 * 这个类包含针对dstram最基本的操作,例如map,filter和window,另外org.apache.spark.streaming.dstream.PairDStreamFunctions
 * 包含只针对key-value简直对的操作例如 groupByKeyAndWindow和join,这些键值对操作对于dstream是可以自动获得的.
